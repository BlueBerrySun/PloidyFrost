...
/**
 * @brief get unitig average coverage and max kmer coverage from kmc database
 * @param u    unitig
 * @param color_id color id
 * @return  (unitig average coverage,max kmer coverage)
 */
pair<double, uint> CCDBG::readCov(const UnitigColorMap<MyUnitig> &u, const size_t &color_id)
{
    double sum = 0;
    CKmerAPI kmer_object(cdbg.getK());
    uint count = 0;
    ...
    if (kmer_databas_vector[color_id]->GetBothStrands())
    {
        for (int i = 0; i < u.len; i++)
        {
            ...
            if (kmer_databas_vector[color_id]->CheckKmer(kmer_object, count))
            {
                if (i != 0)
                {
                    uint d = abs((int)count - (int)before);
                    diff = std::max((diff), d);
                }
                before = count;
                sum += double(count);
            ...
            {
                cout << "CCDBG::readCov():" << kmer_object.to_string() << " kmer can not find in db" << color_id << endl;
                exit(EXIT_FAILURE);
            }
        }
    }
    ...
    return pair<double, uint32>(sum / u.len, diff);
}
...
void CCDBG::ploidyEstimation_multithread_ptr(const string &outpre, const vector<pair<int, int>> &cutoff, const size_t &thr)
{
    if (thr > 1)
    {
        ...
        auto max_num = [](vector<unsigned short> v) -> int
        {
                    ...
                    while (!u.getData()->getData(u)->is_both_visited())
                    {
                        ...
                        {                
                            ...
                            {                               
                                ...
                                if (flag == true)
                                {
                                    for (size_t i = 0; i < path_color.size() - 1; i++)
                                    {
                                        size_t site = i;
                                        for (size_t j = i + 1; j < path_color.size(); j++)
                                        {
                                            if (path_color[j] > path_color[site])
                                            {
                                                site = j;
                                            }
                                            else if (path_color[j] == path_color[site])
                                            {
                                                if(unitig_vec[j].referenceUnitigToString().length()>unitig_vec[site].referenceUnitigToString().length())
                                                {
                                                    site = j;
                                                }
                                                else if (strcmp(unitig_vec[j].referenceUnitigToString().c_str(), unitig_vec[site].referenceUnitigToString().c_str()) > 0)
                                                {
                                                    site = j;
                                                }
                                            }
                                        ...
                                        path_color[i] = path_color[site];
                                        path_color[site] = t;
                                        UnitigColorMap<MyUnitig> u = unitig_vec[site];
                                        unitig_vec[site] = unitig_vec[i];
                                        unitig_vec[i] = u;
                                        for(int k=0;k<cov_vec.size();k++){
                                            t=cov_vec[k][i];
                                            cov_vec[k][i]=cov_vec[k][site];
                                            cov_vec[k][site]=t;
                                        }
                                    }
                                    ...
                                    {        
                                        ...
                                        for (uint i = 0; i < var_site.size(); i++)
                                        {
                                            ...
                                            unsigned short maxnum = max_num(part);
                                            if (i == 0)
                                            {
                                                if (i != var_site.size() - 1)
                                                {
                                                    var_distance =(var_site[i + 1]-var_site[i]-1)<u.size?(var_site[i + 1]-var_site[i]-1):u.size;
                                                }
                                                else
                                                {
                                                    var_distance = u.size < exit_uni.size ? u.size : exit_uni.size;
                                                }
                                            }
                                            else if (i == var_site.size() - 1)
                                            {
                                                var_distance = (var_site[i] - var_site[i-1]  - 1)<exit_uni.size? (var_site[i] - var_site[i-1]  - 1):exit_uni.size;
                                            }
                                            else
                                            {
                                                uint left=var_site[i] - var_site[i-1]-1;
                                                uint right=var_site[i+1]-var_site[i]-1;
                                                var_distance=left<right?left:right;
                                            }
                            ...
                            else
                        {
                            ...
                            for(size_t i=0;i<str_vec.size()-1;i++){
                                ...
                                for(size_t j=i+1;j<str_vec.size();j++){
                                    if(str_vec[j].length()>str_vec[site].length())
                                    {
                                        site=j;
                                        
                                    }
                                    else if (str_vec[j].length()==str_vec[site].length())
                                        if(strcmp(str_vec[j].c_str(),str_vec[site].c_str())>0)
                                        {
                                            site=j;
                                        }
                                }
                                string s=str_vec[site];
                                str_vec[site]=str_vec[i];
                                str_vec[i]=s;
                            }
                            SeqAlign seqalign(match,mismatch,gap);
                            ...
                            if (str_vec.size() != 0)
                            {
                                ...
                                for (uint i = 0; i < var_site.size(); i++)
                                {
                                    ...
                                    stringstream fre_info; 
                                    uint var_distance = 0;
                                    unsigned short maxnum = max_num(partition[var_site[i]]);
 ...
 * @return  (unitig average coverage,max kmer coverage)
 */
pair<double, uint32> CCDBG::readCov(const UnitigColorMap<MyUnitig> &u)
{
    double sum = 0;
    CKmerAPI kmer_object(cdbg.getK());
    uint32 count = 0;
    ...
    return pair<double, uint32>(sum / u.len, max);
}

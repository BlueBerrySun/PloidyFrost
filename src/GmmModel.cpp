...
void GmmModel::sort(vector<double> &vec)
{
...
void GmmModel::emStep()
{
    ...
    vector<double> sort_weights;
    for (int i = 0; i < gauss ; i++)
    {
        sort_weights.push_back(new_weights[i]);
    }
    sort(sort_weights);
    if (sort_weights.back() != new_weights[0] && sort_weights.back() != new_weights[gauss-1])
    {
        if (sort_weights[0] < (double)1 / gauss / m_thre)
        {
            return;
        }
        if (new_weights[0] < (double)sort_weights.back() / gauss / n_thre)
        {
            return;
        }

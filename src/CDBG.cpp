...
pair<double, bool> CDBG::readCov_bool(const UnitigMap<MyUnitig> &u, const uint &low, const uint &up)
{
    double sum = 0;
...
void CDBG::setNoBubble_ptr(pair<UnitigMap<MyUnitig>, UnitigMap<MyUnitig>> &p, const vector<UnitigMap<MyUnitig>> &vec_km_seen)
{
    ...
    // if (p.second.getData()->get_plus_or_minus(!p.second.strand) != NULL || p.second.getData()->is_non_super())
                ...
                if (flag == true)
                {
                    ...
                    for (size_t i = 0; i < cov.size() - 1; i++)
                    {
                        ...
                        for (size_t j = i + 1; j < cov.size(); j++)
                        {
                            ...
                            else if (cov[j] == cov[site])
                            {
                                if (strcmp(unitig_vec[j].referenceUnitigToString().c_str(), unitig_vec[site].referenceUnitigToString().c_str()) > 0)
                                {
                                    site = j;
                                }
                            }
                        ...
                        cov[i] = cov[site];
                        cov[site] = t;
                        UnitigMap<MyUnitig> u = unitig_vec[site];

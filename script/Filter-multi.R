...
option_list <- list(
  ...
  make_option(c("-c", "--color"), type = "integer", default = -1,
              action = "store", help = "sample color"),
  ...
  make_option(c("-v", "--cramer"), type = "double", default = 0.25,
              action = "store", help = "Cramer'V")
)
...
if(file.exists(paste0(file.prefix,"_bicov.txt"))){
  bifile <- read.table(paste0(file.prefix,"_bicov.txt"),
                       col.names = c("CovA","CovB","color","isStrict","VarType","VarId",
                                     "VarNum","Cramer","VarDis"),stringsAsFactors = FALSE,
                       colClasses = c("CovA"="numeric","CovB"="numeric","color"="numeric","Cramer"="numeric"))
}else{
...
if(opt$color<0)
{
  bifile <- bifile[bifile$Cramer>opt$cramer&
                     bifile$CovA>opt$low&bifile$CovB>opt$low&bifile$CovA<opt$up&bifile$CovB<opt$up&			
                     bifile$VarNum<opt$num&bifile$VarDis>opt$distance&bifile$VarType<opt$size,]
  trifile <- trifile[trifile$Cramer>opt$cramer&
                       trifile$CovA>opt$low&trifile$CovB>opt$low&trifile$CovC>opt$low&trifile$CovA<opt$up&trifile$CovB<opt$up&trifile$CovC<opt$up&
                       trifile$VarNum<opt$num&trifile$VarDis>opt$distance&trifile$VarType<opt$size,]
  tetrafile <- tetrafile[tetrafile$Cramer>opt$cramer&
                           tetrafile$CovA>opt$low&tetrafile$CovB>opt$low&tetrafile$CovC>opt$low&tetrafile$CovD>opt$low&tetrafile$CovA<opt$up&tetrafile$CovB<opt$up&tetrafile$CovC<opt$up&tetrafile$CovD<opt$up&
                           tetrafile$VarNum<opt$num&tetrafile$VarDis>opt$distance&tetrafile$VarType<opt$size,]
  pentafile <- pentafile[pentafile$Cramer>opt$cramer&
                           pentafile$CovA>opt$low&pentafile$CovB>opt$low&pentafile$CovC>opt$low&pentafile$CovD>opt$low&pentafile$CovE>opt$low&
                           pentafile$CovA<opt$up&pentafile$CovB<opt$up&pentafile$CovC<opt$up&pentafile$CovD<opt$up&pentafile$CovE<opt$up&
                           pentafile$VarNum<opt$num&pentafile$VarDis>opt$distance&pentafile$VarType<opt$size,] 
}else{
  bifile <- bifile[bifile$Cramer>opt$cramer&bifile$color==opt$color&
                     bifile$CovA>opt$low&bifile$CovB>opt$low&bifile$CovA<opt$up&bifile$CovB<opt$up&
                     bifile$VarNum<opt$num&bifile$VarDis>opt$distance&bifile$VarType<opt$size,]
  trifile <- trifile[trifile$Cramer>opt$cramer&trifile$color==opt$color&
                       trifile$CovA>opt$low&trifile$CovB>opt$low&trifile$CovC>opt$low&trifile$CovA<opt$up&trifile$CovB<opt$up&trifile$CovC<opt$up&
                       trifile$VarNum<opt$num&trifile$VarDis>opt$distance&trifile$VarType<opt$size,]
  tetrafile <- tetrafile[tetrafile$Cramer>opt$cramer&tetrafile$color==opt$color&
                           tetrafile$CovA>opt$low&tetrafile$CovB>opt$low&tetrafile$CovC>opt$low&tetrafile$CovD>opt$low&tetrafile$CovA<opt$up&tetrafile$CovB<opt$up&tetrafile$CovC<opt$up&tetrafile$CovD<opt$up&
                           tetrafile$VarNum<opt$num&tetrafile$VarDis>opt$distance&tetrafile$VarType<opt$size,]
  pentafile <- pentafile[pentafile$Cramer>opt$cramer&pentafile$color==opt$color&
                           pentafile$CovA>opt$low&pentafile$CovB>opt$low&pentafile$CovC>opt$low&pentafile$CovD>opt$low&pentafile$CovE>opt$low&
                           pentafile$CovA<opt$up&pentafile$CovB<opt$up&pentafile$CovC<opt$up&pentafile$CovD<opt$up&pentafile$CovE<opt$up&
                           pentafile$VarNum<opt$num&pentafile$VarDis>opt$distance&pentafile$VarType<opt$size,] 
}

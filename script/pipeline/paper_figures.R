...
total_num <- dim(list_read[[1]])[1]
filt_num1 <- dim(coverage_num5)[1]
filt_num2 <- dim(coverage_num1)[1]
filt_num1/total_num
filt_num2/total_num
...
#####LSX118####
LSX118dir <- ""
list_read <- readcov(LSX118dir) #18
cov <- 18
p <- 4
v <- c()
if(p>=2){
...
#####Fragaria ¡Á ananassa####
Fanadir <- "E:/data/Fragaria¡Áananassa/PloidyFrost_output/Fana"
Fana <- readcov(Fanadir) #cov52
cov <- 52
p <- 8
v <- c()
if(p>=2){
...
SNJdir <- ""
list_read <- colour.readcov(SNJdir)
cov1 <- 39.3
cov2 <- 40
cov3 <- 45.8
...
frequency_num5 <- frequency[frequency$VarNum<=5&frequency$VarSize<=10,]
frequency_num5$type <- "VarNum<=5&VarSize<=10"
frequency_gt <- frequency[frequency$coe>=0.25,]
frequency_gt$type <- "Cramer's V >= 0.25"
frequency_lt <- frequency[frequency$coe<0.25,]
frequency_lt$type <- "Cramer's V < 0.25"
frequency <- rbind(frequency,frequency_num5)
frequency <- rbind(frequency,frequency_gt)
frequency <- rbind(frequency,frequency_lt)
...
coverage_num5 <- coverage[coverage$VarNum<=5&coverage$VarSize<=10,]
coverage_num5$type <- "VarNum<=5&VarSize<=10"
coverage_gt <- coverage[coverage$coe>=0.25,]
coverage_gt$type <- "Cramer's V >= 0.25"
coverage_lt <- coverage[coverage$coe<0.25,]
coverage_lt$type <- "Cramer's V < 0.25"
coverage <- rbind(coverage,coverage_num5)
coverage <- rbind(coverage,coverage_gt)
coverage <- rbind(coverage,coverage_lt)


total_num17 <- dim(list_read[[1]][list_read[[1]]$color==0,])[1]
total_num18 <- dim(list_read[[1]][list_read[[1]]$color==1,])[1]
total_num110 <- dim(list_read[[1]][list_read[[1]]$color==2,])[1]

filt_num1.17 <- dim(coverage_num5[coverage_num5$color==0,])[1]
filt_num1.18 <- dim(coverage_num5[coverage_num5$color==1,])[1]
filt_num1.110 <- dim(coverage_num5[coverage_num5$color==2,])[1]

filt_num2.17 <- dim(coverage_gt[coverage_gt$color==0,])[1]
filt_num2.18 <- dim(coverage_gt[coverage_gt$color==1,])[1]
filt_num2.110 <- dim(coverage_gt[coverage_gt$color==2,])[1]

filt_num1.17/total_num17
filt_num1.18/total_num18
filt_num1.110/total_num110

filt_num2.17/total_num17
filt_num2.18/total_num18
filt_num2.110/total_num110


#sites with coverages outside the expected coverage range
total_num2.17 <- dim(list_read[[1]][list_read[[1]]$color==0&(list_read[[1]]$coverage<cov1|list_read[[1]]$coverage>cov1*3),])[1]
total_num2.18 <- dim(list_read[[1]][list_read[[1]]$color==1&(list_read[[1]]$coverage<cov2|list_read[[1]]$coverage>cov2*3),])[1]
total_num2.110 <- dim(list_read[[1]][list_read[[1]]$color==2&(list_read[[1]]$coverage<cov3|list_read[[1]]$coverage>cov3*3),])[1]

coverage_num <- list_read[[1]][list_read[[1]]$VarNum>5|list_read[[1]]$VarSize>10,]
dim(coverage_num[coverage_num$color==0&(coverage_num$coverage<cov1|coverage_num$coverage>cov1*3),])[1]/total_num2.17
dim(coverage_num[coverage_num$color==1&(coverage_num$coverage<cov2|coverage_num$coverage>cov2*3),])[1]/total_num2.17
dim(coverage_num[coverage_num$color==2&(coverage_num$coverage<cov3|coverage_num$coverage>cov3*3),])[1]/total_num2.17

total_lt17 <- dim(coverage_lt[coverage_lt$color==0&(coverage_lt$coverage<cov1|coverage_lt$coverage>cov1*3),])[1]
total_lt18 <- dim(coverage_lt[coverage_lt$color==1&(coverage_lt$coverage<cov2|coverage_lt$coverage>cov2*3),])[1]
total_lt110 <- dim(coverage_lt[coverage_lt$color==2&(coverage_lt$coverage<cov3|coverage_lt$coverage>cov3*3),])[1]
#
